...
- Database & Models
  - [ ] Add `label TEXT NULL` to `job_executions` (schema migration) for human-friendly job run names.
  - [ ] Ensure indices exist on `started_at`, `status`, and `configuration_id`; keep transactional consistency for deletes.

- Backend Adapter & IPC
  - [ ] Implement IPC handlers: `job-execution:rename`, `job-execution:bulk-delete`, `job-execution:bulk-export`, `job-execution:bulk-rerun`.
  - [ ] Preload exposure on `window.electronAPI` for new endpoints; add to whitelist.
  - [ ] Validate and clamp inputs; translate errors consistently.

- Renderer UI — Dashboard Overview
  - [ ] Keep Dashboard `JobHistory` as overview-only; each row opens Single Job View.
  - [ ] Remove/avoid batch selection from Dashboard.
  - [ ] Ensure quick actions (View, Export, Delete, Rerun) remain.

- Single Job View
  - [ ] Build Single Job View (modal or dedicated page) with details, images, rename, export, rerun, delete.
  - [ ] Deep-link from Dashboard overview and Job Management table.

- Job Management Page
  - [ ] Create `Jobs/JobManagementPanel.tsx` with table, filters, search, pagination, multi-select.
  - [ ] Implement batch actions: delete (with confirmation), export (per-job Excel), rerun (queued sequentially).
  - [ ] Inline rename for label with optimistic update and rollback on failure.
  - [ ] Show queue indicator when reruns are scheduled; enforce single-job constraint.

- Retry Executor (Post-processing only; no generation)
  - Backend
    - [ ] Extend `retryFailedImagesBatch(imageIds, useOriginalSettings, modifiedSettings)` to trigger a post-processing executor immediately (when idle) or queue in-memory if a job is running.
    - [ ] Resolve source files from Settings `tempDirectory` (fallback `./picture/generated`) via each image's `finalImagePath`.
    - [ ] Original settings: resolve post-processing subset from the originating execution; disallow cross-execution batches.
    - [ ] Modified settings: apply sanitized batch settings; add optional `includeMetadata` flag to regenerate metadata.
    - [ ] Do not invoke prompt/image generation during Retry; only post-processing (+ optional metadata).
    - [ ] Emit progress via existing job channels with `context: 'retry'`; translate errors via `errorTranslation`.
    - [ ] Update `qcStatus` transitions per image (retry_pending → processing → approved/failed); persist outcomes.
  - IPC / Preload
    - [ ] Extend `failed-image:retry-batch` payload to accept `{ includeMetadata?: boolean }` (or add a companion channel); whitelist in `electron/preload.js`.
  - UI (Failed Images Review / Modal)
    - [ ] Update `ProcessingSettingsModal` to include a "Also regenerate metadata" checkbox.
    - [ ] Disable Retry actions when another job is running; show queued indicator when applicable.
    - [ ] After starting Retry, surface progress and final outcomes; refresh failed images list.
  - Preconditions & Guards
    - [ ] Retry available only after at least one full job completed; validate selected images belong to completed execution(s).
    - [ ] Enforce original-settings path only for single-execution selections; otherwise require modified settings.
  - Tests
    - [ ] Unit: settings resolution (original vs modified), `includeMetadata` handling, queue behavior, qcStatus transitions.
    - [ ] Integration: post-processing against file paths (mocked IO), progress events, DB updates.
    - [ ] E2E: Failed Images Review with metadata checkbox; progress to completion and UI refresh.

- Accessibility & Performance
  - [ ] ARIA roles/labels for tables, selection, and batch actions; full keyboard flows.
  - [ ] Virtualization/pagination for large result sets; debounce filters/search.

- Testing
  - [ ] Unit: selection logic, rename, bulk ops; IPC adapters; DB migration tests.
  - [ ] Integration: bulk operations, sequential rerun, transactional deletes.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ ← Back | Job Management                    | [Refresh] [Help]      │
├─────────────────────────────────────────────────────────────────────┤
│ [Total: 45] [Completed: 38] [Failed: 3] [Running: 2] [Pending: 2] │
├─────────────────────────────────────────────────────────────────────┤
│ [Status ▼] [Date Range] [Config ▼] [Search Jobs...] [Clear All]   │
├─────────────────────────────────────────────────────────────────────┤
│ Job Table with multi-selection and pagination                      │
├─────────────────────────────────────────────────────────────────────┤
│ [5 jobs selected] [Rerun] [Export] [Delete] [Clear Selection]     │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ ← Back | Job Details: Product Photos                    | [Actions] │
├─────────────────────────────────────────────────────────────────────┤
│ [Overview] [Images] [Logs]                                        │
├─────────────────────────────────────────────────────────────────────┤
│ Job Info: ID, Status, Duration, Success Rate                       │
│ Generated Images: Total, Successful, Failed                        │
│ Processing Parameters: Settings used                               │
├─────────────────────────────────────────────────────────────────────┤
│ Images Grid (3 columns) with QC status indicators                  │
│ [Load More] [Export All] [Filter by Status]                       │
└─────────────────────────────────────────────────────────────────────┘
```
...
### Agent Model Used
Scrum Master (BMAD method) → Story Preparation
