...
- `.github/workflows/ci.yml` - Created comprehensive CI workflow with build, tests, CodeQL, Semgrep, Socket.dev (enhanced with JSON output, jq parsing, SARIF validation)
- `.semgrep.yml` - Created centralized Semgrep configuration with precise include paths and comprehensive exclusions
...
- `docs/architecture/testing-strategy.md` - Updated with pre-commit gate details, CI workflow documentation, and centralized Semgrep configuration
...
**Centralized Semgrep Configuration**:
- Created `.semgrep.yml` with centralized ruleset configuration (p/owasp-electron, p/owasp-top-ten, p/javascript, p/typescript)
- Ensures consistent security scanning across local development, pre-commit hooks, and CI
- Updated `.semgrepignore` with comprehensive exclusion patterns (build artifacts, test files, media assets)
- Used precise include paths in `.semgrep.yml` (explicitly including `electron/main.js` and `electron/preload.js` instead of entire `electron/` directory)
- This approach eliminates false positives from build output (`electron/renderer/`) and test files
