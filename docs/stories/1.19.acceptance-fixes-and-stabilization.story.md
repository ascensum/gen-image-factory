...
- [x] QC per‑image timeout
  - [x] Wrap `aiVision.runQualityCheck` with `withTimeout`; use Generation Timeout when enabled or fallback to 30s per image
...
- Doc reference: Runware Image Inference API — LoRA parameter (`lora` array).
