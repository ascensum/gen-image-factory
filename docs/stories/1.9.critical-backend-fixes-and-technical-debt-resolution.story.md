...
#### **Task 1.9.16: Harden local-file protocol and cross-platform path handling (CRITICAL)**
- [x] **Status:** COMPLETED
- [x] **Implementation:** Resolved macOS privacy reset breaks and cross-platform path issues by adding an allowlist + one-time permission prompt and robust URL/path normalization.
- [x] **Code Changes:**
  - `electron/main.js`:
    - Added in-memory allowed roots seeded from Desktop/Documents, saved Settings paths, and recent image parent folders.
    - Added macOS one-time folder access prompt when TCC denies reads; grants then serve seamlessly.
    - Enforced read-access checks before serving; serve only under allowed roots.
    - Fixed UNIX URL host handling (e.g., `local-file://users/...`) by reconstructing the full path.
    - Normalized Windows paths by stripping leading slash from `/C:/...`.
    - Added IPC `protocol:refresh-roots` and `protocol:request-access`.
  - `electron/preload.js`:
    - Exposed `refreshProtocolRoots(extraPaths)` and `requestFileAccess(filePath)` for renderer to proactively grant/refresh access after Settings changes.
- [x] **Commit:** `fix(protocol): macOS one-time folder access prompt + allowed roots; cross-platform URL/path normalization`
- [x] **Impact:** 
  - Images render reliably after system cleaners or privacy resets without crashes.
  - Works in dev and packaged builds across macOS/Windows/Linux.
  - No repeated prompts; only when switching to a new folder or after permissions reset.
