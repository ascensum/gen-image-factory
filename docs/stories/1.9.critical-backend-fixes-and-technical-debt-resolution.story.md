...
- Task 1.9.30: Settings Parity and Type Alignment (UI + Backend)
  ...
  - Subtasks (Status):
    ...
    - [ ] Normalizer: Promote `sanitizeProcessingSettings` to `src/utils/processing.ts` and export `normalizeProcessingSettings` (partial: backendAdapter clamp to 1–100 implemented)
  ...
  - Tests (Progress):
    ...
    - [ ] Integration (Labels): Stabilize and re-enable label regression tests in pre-commit
      - [ ] Fix test stubbing so `JobRunner` is reliably mocked (avoid OpenAI/PIAPI calls)
      - [ ] Re-enable the following in Husky once stabilized:
        - `FallbackLabelUnification.integration.test.ts`
        - `LabelNoOverwriteOnCompletion.integration.test.ts`
        - `RenameUpdatesConfiguration.integration.test.ts`
