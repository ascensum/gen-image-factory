...
Status: PLANNED → IN PROGRESS (incremental commits forthcoming)
...
- Task 1.9.30: Settings Parity and Type Alignment (UI + Backend)
  ...
  - Subtasks (Planned):
    - [ ] Types: Extend `JobConfiguration.processing` with `imageEnhancement`, `sharpening`, `saturation` (TS-only)
    - [ ] Types: Create shared `ProcessingSettings` in `src/types/processing.d.ts` and import in Retry/Failed Review and Settings/Single Job Edit
    - [ ] Normalizer: Promote `sanitizeProcessingSettings` to `src/utils/processing.ts` and export `normalizeProcessingSettings`
    - [ ] Normalizer Call-sites: Apply normalization at boundaries
    ...
    - [ ] UI Parity: Single Job View “Edit Job Settings”
      - [ ] Add missing Parameters: `openaiModel`, `pollingTimeout`, `pollingInterval`, `enablePollingTimeout`, `keywordRandom`, `count`
      - [ ] Add missing File Paths: `systemPromptFile`, `keywordsFile`, `qualityCheckPromptFile`, `metadataPromptFile`
      - [ ] Reuse `Toggle` and `FileSelector` for consistency
    - [ ] UI Consistency: Retry Processing Settings modal
      - [ ] Import shared `ProcessingSettings`
      - [ ] Ensure numeric/boolean emission (parseInt/parseFloat/Boolean)
    - [ ] Display: Single Job View Overview
      - [ ] Show `keywordRandom` and key parameters for clarity
    - [ ] Range Standardization
      - [ ] Standardize `pngQuality` to 1–100 across UI/backend; set Retry modal default to 100
  - Tests (Planned):
    - [ ] Unit: Normalizer clamps & coercions (strings→numbers; `pngQuality` 1–100; enum fallbacks)
    - [ ] Integration: Retry/Rerun with stringified values → normalized at processing
    - [ ] Integration: Single Job Edit saves parity fields; Retry modal emits numbers
