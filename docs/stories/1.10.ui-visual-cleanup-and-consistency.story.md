...
23. Rerun label format is unified across the entire UI as `Parent Label (Rerun execIdShort)`, including during In‑Progress state (Job History, Job Management, Single Job View, Image Gallery, Failed Images Review)
24. Label formatting regression tests exist and are enforced by the pre‑commit hook; commits fail if rerun labels regress
25. The single‑job constraint message is always visible above the Logs Panel when scrolling (correct z‑index and shadow)
26. Filter affordances are consistent: menu‑like dropdowns show a right‑aligned chevron; Job Management Status filter uses the same affordance
27. Job filters in Image Gallery and Failed Images Review display reruns with `execIdShort` without horizontal scroll; long labels wrap
28. Failed Images Review counters auto-refresh after "Retry with Original settings"; Retry Pending and Processing counts settle without manual reload
...
- Single Job View edit parity
  ...
  - [x] Edit Settings UX updated for Runware provider:
    - [x] Reordered Parameters logically (generation-first): Name/Label → Runware Model → Dimensions (W×H or CSV) → Output Format → Generations count → Variations → Enable Generation Timeout → Generation Retry Attempts → Retry Backoff (ms) → Runware Advanced Controls toggle (reveals CFG Scale, Steps, Scheduler) → OpenAI Model → Random Keywords
    - [x] Removed legacy Midjourney fields (Process Mode, Aspect Ratios, MJ Version); removed Polling Interval for Runware; kept OpenAI Model for QC/Metadata; API keys are settings-only (note shown)
    - [x] Dropdowns/selects now full-width to match text inputs and prevent modal overflow
  - [x] After saving, Single Job View refreshes jobConfiguration so reopening Edit Settings reflects the latest values
...
- Inputs behavior
  ...
  - [x] Single Job View: show Generation Timeout/Interval only when "Enable Generation Timeout" is ON (parity with Settings)
...
### Additional tasks added during implementation (Done)

- Labeling and naming consistency
  - [x] Unify rerun label display across UI as `Parent Label (Rerun execIdShort)` (Dashboard Job History, Job Management list, Image Gallery job filter, Failed Images Review job filter)
  - [x] Add rerun label regression tests (unit) and gate via pre‑commit hook

- Filters and controls UX harmonization
  - [x] Add bold right‑aligned chevron to menu‑like dropdowns via `SimpleDropdown`; apply to Job Management Status filter
  - [x] Ensure job filter dropdowns format reruns with `execIdShort` in Image Gallery and Failed Images Review
  - [x] Prevent horizontal scroll in job filter dropdowns; wrap long labels to multiple lines

- Overlay/Z‑index polish
  - [x] Layer the single‑job constraint message above Logs Panel using `z-50` and `shadow-lg` so it remains visible while scrolling

- Counters correctness (Failed Images Review)
  - [x] After retry with original settings, run a short-lived refresh loop to settle Retry Pending/Processing counters; stops automatically when selected images leave those states

- Retry Processing Settings UX (modal)
  - [x] Compact layout for "Retry with Original" and "Retry with Modified" options using two card-like radio choices
  - [x] Removed distracting blue focus outline/border from radio inputs within the cards
  - [x] Fixed header/footer; content area scrolls vertically with a visible right-side scrollbar (no auto-scroll jumps)
  - [x] Prevented footer button overflow; ensured buttons remain fully visible at all times
  - [x] Kept manual scroll control when toggling options or expanding sections (Enhancement/Conversion/Background)
  - [x] Aligned JPG Background Color options to White/Black only (removed Gray/Light Gray); matched Single Job View edit modal and Settings/backend
...
- Job managment pannel statistics, progress bars, indicators, filters improvements
  ...
  - [x] Added "Has pending retries" filter (UI-only) to surface parents queued for reruns; removed "Pending" from Status dropdown; removed blue focus ring from the new checkbox
 - [x] Image Gallery: Job filter is now searchable with a scrollable dropdown (better UX for long job lists)
 - [x] Image Gallery: Job filter is now searchable with a scrollable dropdown (better UX for long job lists); rerun jobs display as `Parent Label (Rerun execIdShort)` in selection and list
...
- Failed images review UI (finalized)
  ...
  -  [x] Job filter: searchable dropdown with scrollable list; shows job label or fallback
 -  [x] Job filter: searchable dropdown with scrollable list; shows job label or fallback; rerun jobs display as `Parent Label (Rerun execIdShort)`
  ...
  -  [x] Ribbon layout parity with Image Gallery: filters on left (View → Job → Search → Sort → Clear), bulk actions on right (Select All + Approve/Retry/Delete)
...
- Electron application icon replacement (convert favicon.svg to platform-specific formats)
  ...
  - [x] Generated files should include:
    - [x] **macOS**: `icon.icns` (contains 16, 32, 64, 128, 256, 512, 1024px sizes)
...
| 2025-10-25 | 1.12    | Failed Images Review: searchable Job filter dropdown; ribbon controls reordered to match Image Gallery | Dev |
| 2025-10-25 | 1.13    | Retry Processing Settings: compact two-card radios, removed blue focus outline, visible scrollbar on content, fixed header/footer (no overflow), removed auto-scroll to keep user control | Dev |
| 2025-10-25 | 1.14    | Single Job View Edit Settings: reordered Parameters for generation flow; added Runware Advanced Controls toggle; selects full-width; removed MJ fields and Polling Interval; kept OpenAI Model; Job Management: added "Has pending retries" filter and removed "Pending" from Status | Dev |
| 2025-10-25 | 1.15    | Single Job View: saving settings now refreshes local configuration; reopening Edit reflects latest values | Dev |
| 2025-10-25 | 1.16    | Rerun naming parity: backend prefers updated Single Job View label (parameters.label) or configuration name when building rerun label; falls back to previous execution label; always appends "(Rerun)" without duplicating suffix | Dev |
| 2025-10-28 | 1.17    | Rerun display format unified across UI: Job History, Job Management, Image Gallery, and Failed Images Review show `Parent Label (Rerun execIdShort)`; added label regression tests and pre-commit check | Dev |
| 2025-10-28 | 1.18    | Dashboard: single‑job constraint message layered above Logs Panel (z-50, shadow) to prevent overlap on scroll | Dev |
| 2025-10-29 | 1.19    | Job Management: Status filter shows bold chevron; `SimpleDropdown` updated to include right-aligned chevron for consistent filter affordance | Dev |
| 2025-10-29 | 1.20    | Failed Images Review: counters settle after retry-original via short-lived polling refresh; added AC and completed task | Dev |
