...
- Home screen branding and simplification
  ...
  - [x] Add centered banner logo at top (import from assets)
...
- Main panel header branding
  - [ ] Add main logo to Dashboard panel header (`DashboardPanel.tsx`):
    - [ ] Place logo in center of header area without displacing menu button or existing controls
    - [ ] Size logo appropriately (suggested: h-8 to h-10, ~32-40px height)
    - [ ] Ensure logo doesn't overlap with HeaderMenu or tab navigation
  - [ ] Add main logo to Job Management panel header (`JobManagementPanel.tsx`):
    - [ ] Center logo in header without breaking Back button or search/filter controls
    - [ ] Maintain consistent size with Dashboard logo
    - [ ] Test at narrow widths to ensure no overflow
  - [ ] Add main logo to Settings panel (`SettingsPanel.tsx`):
    - [ ] Place logo at top center of each tab page, above form sections
    - [ ] Position before "API Keys", "Parameters", "File Paths", "Quality Control" sections
    - [ ] Use consistent sizing and spacing (suggested: mb-6 margin below logo)
    - [ ] Ensure logo doesn't conflict with Back/Dashboard navigation buttons
  - [ ] Add main logo to Failed Images Review panel header (`FailedImagesReviewPanel.tsx`):
    - [ ] Center logo in header area without displacing Back button or filter controls
    - [ ] Match sizing with other panel logos
    - [ ] Verify logo placement doesn't affect grid/list view toggle or bulk actions
  - [ ] Create reusable `<AppLogo>` component (optional but recommended):
    - [ ] Accept size prop (sm, md, lg) for different contexts
    - [ ] Apply consistent styling and accessibility attributes
    - [ ] Import and use across all panels for maintainability
  - [ ] Test logo rendering across all panels at multiple screen widths
  - [ ] Verify no layout shifts or element displacement when logo is added

- Electron application icon replacement (convert favicon.svg to platform-specific formats)
  - [ ] Create icon assets directory: `/build/icons/` (standard Electron location)
  - [ ] Convert `favicon.svg` to platform-specific formats using automated tool:
    - [ ] **Recommended tool**: `electron-icon-builder` (one command converts to all formats)
      ```bash
      npm install -g electron-icon-builder
      electron-icon-builder --input=./src/renderer/assets/favicon/favicon.svg --output=./build/icons
      ```
    - [ ] **Alternative**: Use online tool like https://cloudconvert.com or manual conversion
  - [ ] Generated files should include:
    - [ ] **macOS**: `icon.icns` (contains 16, 32, 64, 128, 256, 512, 1024px sizes)
    - [ ] **Windows**: `icon.ico` (contains 16, 32, 48, 64, 128, 256px sizes)
    - [ ] **Linux**: Individual PNG files (16, 32, 64, 128, 256, 512px) - use 512x512.png as main
  - [ ] Update Electron main process (`electron/main.js`):
    - [ ] Add `icon` property to `BrowserWindow` constructor (around line 61)
    - [ ] Use platform detection to load correct icon:
      ```javascript
      let iconPath;
      ...
      ```
  - [ ] Update `package.json` build configuration (if using electron-builder for distribution):
    - [ ] Add `"mac": { "icon": "build/icons/icon.icns" }`
    - [ ] Add `"win": { "icon": "build/icons/icon.ico" }`
    - [ ] Add `"linux": { "icon": "build/icons/512x512.png" }`
  - [ ] Update `index.html` favicon reference (for browser tab in dev mode):
    - [ ] Add `<link rel="icon" type="image/svg+xml" href="/src/renderer/assets/favicon/favicon.svg" />` in `<head>`
  - [ ] Test icon appearance on all target platforms:
  ...
  - [ ] Note: Banner logo and main panel logo remain as `.svg` format (no conversion needed)
