# Story 1.16: Testing - Implement Comprehensive Testing for Story 1.1
...
- [ ] Create integration tests for IPC communication
  ...
  - [ ] Export IPC: validate `generated-image:export-zip` progress events and success payload
  - [ ] Export IPC: validate `job-execution:export-to-excel` and `job-execution:bulk-export` accept `{ options }` and return file paths
...
- [ ] Create end-to-end tests for basic app functionality
  ...
  - [ ] Export UI: verify `ExportFileModal` remembers last custom path per type (xlsx, zip)
  - [ ] Export UI: verify duplicate policy append-number and overwrite behaviors via UI flows
  - [ ] StatusBadge rendering consistency: verify failed badges (including `retry_failed`) use red style and exclamation icon when `labelOverride` is provided
