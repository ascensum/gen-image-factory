# Story 1.13: Security Hardening - Implement Basic Security Measures for Single User
...
### Export-Specific Security Tests (Planned)
- Validate export IPC channels accept only whitelisted options (outputPath, duplicatePolicy) and reject unexpected fields
- Ensure exports never include sensitive settings (API keys) in Excel or ZIP contents
- Verify path sanitization prevents directory traversal when saving to custom locations
