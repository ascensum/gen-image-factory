...
const tempDir = path.join(os.tmpdir(), `gif-zip-export-test-${Date.now()}`);
...
describe('ZIP Export - Integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Create two dummy image files and one missing
    ...
    // Wire generatedImage.getGeneratedImage to return paths
  ...
  it('creates a ZIP file with images and metadata', async () => {
    ...
    if (!res.success) {
      ...
      console.error('ZIP export failure:', res);
    }
  ...
  it('respects overwrite duplicate policy for custom output path', async () => {
    ...
    // Pre-create file and ensure policy overwrite replaces it
