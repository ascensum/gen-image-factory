/**
 * Story 3.4 Phase 5c: Zoom/pan state and handlers for ImageModal.
 * Extracted from ImageModal.tsx to keep modal < 400 lines.
 */
import { useState, useRef, useCallback, useEffect } from 'react';

export interface UseImageModalZoomPanOptions {
  isOpen: boolean;
  imageId: string | number | null;
}

export function useImageModalZoomPan({ isOpen, imageId }: UseImageModalZoomPanOptions) {
  ...
  // React's onWheel is passive by default, which causes "Unable to preventDefault inside passive event listener".
  useEffect(() => {
    if (!isOpen) return;
