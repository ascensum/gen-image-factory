...
(async () => {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .option('imageEnhancement', {
      type: 'boolean',
      description: 'Enable or disable image enhancement (sharpening/saturation). Overrides IMAGE_ENHANCEMENT in .env',
    })
    .option('sharpening', {
      type: 'number',
      description: 'Sharpening intensity (0-10). Overrides SHARPENING in .env',
      default: 5,
    })
    .option('saturation', {
      type: 'number',
      description: 'Saturation level (0-2). Overrides SATURATION in .env',
      default: 1.4,
    })
    .option('convertToJpg', {
      type: 'boolean',
  ...
  const config = {
    ...
    sharpening: argv.sharpening !== undefined ? argv.sharpening : (parseFloat(process.env.SHARPENING) || 5),
    saturation: argv.saturation !== undefined ? argv.saturation : (parseFloat(process.env.SATURATION) || 1.4),
